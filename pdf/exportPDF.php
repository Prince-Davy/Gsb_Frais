...
class PDF extends FPDF {
    ...
    function enteteFicheFrais($bdd, $lesMois, $lUtilisateur) {
	...
	$idJeuFicheDeFrais = $bdd->query('select nom, prenom from utilisateur join FicheFrais on id = idUtilisateur where id="' . $lUtilisateur . '" and mois="' . $lesMois . '";');
	...
	// Infos visiteur id + nom et prenom
        $this->Cell(50, 7, "Utilisateur", 0);
        $this->Cell(40, 7, $lUtilisateur, 0);
	...
	// Infos Mois 
        ...
        $mois = intval(substr($lesMois, 4, 2));
        $annee = intval(substr($lesMois, 0, 4));
        
        $this->Cell(40, 7, obtenirLibelleMois($mois) . ' ' . $annee, 0);
    }

	// ____________Tableau Frais Forfait___________
    function tabFraisForfaits($bdd, $lesMois, $lUtilisateur) {
        // Entêtes de colonnes
        $this->Ln(15);
        $this->Cell(10);
        $this->SetTextColor(31, 73, 125);
	...
	// ainsi que le montant, le total (quantite*montant) grace à la jointure avec la table FraisForfait
        $idJeuFraisForfait = $bdd->query("select libelle, quantite, montant, (quantite*montant) as total from LigneFraisForfait inner join FraisForfait on FraisForfait.id = LigneFraisForfait.idFraisForfait where idUtilisateur='" . $lUtilisateur . "' and mois='" . $lesMois . "'");
    ...
    function tabFraisHorsForfaits($bdd, $lesMois, $lUtilisateur) {
        $this->Ln(5);
        $this->Cell(10);
        $this->SetTextColor(31, 73, 125);
	...
	// requete pour récupérer les valeurs de id, date, libelle, montant de la table LigneFraisHorsForfait
        $idJeuFraisHorsForfait = $bdd->query("select id, date, libelle, montant from LigneFraisHorsForfait where idUtilisateur='" .
                $lUtilisateur . "' and mois='" . $lesMois . "'");
	...
	// permet de modifier une date de type Anglais(annee/mois/jour) en type Français(jour/mois/annee)
            $this->Cell(50, 7, dateAnglaisVersFrancais($lgFraisHorsForfait['date']), 1, 0, 'L', true);
    ...
    function afficheTotal($bdd, $lesMois, $lUtilisateur) {
        $this->Ln();
        $this->Cell(100);
        
	// la requete récupère le montant total de la fiche de frais dans la table FicheFrais
        $idJeuFicheFrais = $bdd->query("select montantValide from FicheFrais where idUtilisateur='" . $lUtilisateur . "' and mois='" . $lesMois . "'");
    ...
    function afficheSignature($bdd, $lesMois, $lUtilisateur) {
        $this->Ln(20);
        $this->Cell(100);
        $idJeuFicheFrais = $bdd->query("select dateModif from FicheFrais where idUtilisateur='" . $lUtilisateur . "' and mois='" . $lesMois . "'");
        $lgFicheFrais = $idJeuFicheFrais->fetch();
        $idJeuFicheFrais->closeCursor();
    ...
    function afficheFicheFrais($lesMois, $lUtilisateur) {
        ...
        try {
            $bdd = new PDO('mysql:host=localhost;dbname=gsb_frais', 'root', 'root');
        } catch (Exception $e) {
            die('Erreur : ' . $e->getMessage());
        }

        // Affichage de l'entête de la fiche de frais
        $this->enteteFicheFrais($bdd, $lesMois, $lUtilisateur);
        // Affichage des frais forfaitisés
        $this->tabFraisForfaits($bdd, $lesMois, $lUtilisateur);
        // Affichage des frais hors forfaits
        $this->tabFraisHorsForfaits($bdd, $lesMois, $lUtilisateur);
        // Affichage du total
        $this->afficheTotal($bdd, $lesMois, $lUtilisateur);
        // Affichage de la date et de la signature du document
        $this->afficheSignature($bdd, $lesMois, $lUtilisateur);
    }
...
if (!file_exists($fichier)) {
	...
	$pdf = new PDF();
	$pdf->afficheFicheFrais($lesMois, $lUtilisateur);
